		...
		SuperMatrix *L, SuperMatrix *U, GlobalLU_t *Glu)
{
	...
	if(Glu->xsup != 0 && whichspace == SYSTEM){free(Glu->xsup);}
	if(Glu->xsup_end != 0 && whichspace == SYSTEM)free(Glu->xsup_end);
	if(Glu->supno != 0 && whichspace == SYSTEM)free(Glu->supno);
	if(Glu->xusub_end != 0 && whichspace == SYSTEM)free(Glu->xusub_end);
