...
const int MMC_MAX_EDGE_ELEMS = 101; /* max number of elements containg an edge */
...
//mmt_ref_info mmv_ref_loc;
//mmt_ref_info mmv_ref_ali;
//mmt_del_info mmv_del_loc;
...
extern int mmr_export_mesh( /* returns: >=0 - success code, <0 - error code */
  ...
  int Control,	  /* in: control variable to choose data format */
        /* MMC_MOD_FEM_MESH_DATA - generic mesh for this module */
        /* MMC_MOD_FEM_PRISM_DATA - prisnmatic mesh  */
  char *Filename  /* in: name of the file to read mesh data */
  )
{
...
{
  return (mmr_divide_el4_p(Mesh_id,El)); 
}
...
{
  return(mmr_clust_el4_p(Mesh_id,El));
}
...
/*kew*/
  ...
  for(iel=0;iel<nrel_div;iel++){
    iaux=mmr_divide_el4_p(Mesh_id,list_temp[iel]); 
    if(iaux<0) break;
  }
...
/* select the proper mesh data structure */
  ...
  for(iel=0;iel<nrel_div;iel++){
    ...
    if(mmr_el_status(Mesh_id,list_temp[iel])==MMC_ACTIVE){
      ...
      iaux=mmr_clust_el4_p(Mesh_id,list_temp[iel]);
      if(iaux<0) break;
    }
  }
...
/* select the proper mesh data structure */
  ...
  for(iel=1;iel<=nmel;iel++){
    ...
    if(mmr_el_status(Mesh_id,iel)==MMC_ACTIVE) {
      if(mesh->elem[iel].refi==MMC_REF_ISO) num_sons=8;
      else if(mesh->elem[iel].refi==MMC_REF_ANI) num_sons = 4;
      else num_sons = 0;

      if(num_sons>0){

/*kbw
	printf("Finalizing refinement for el %d, refi %d, num_sons %d\nsons:", 
	       iel, mesh->elem[iel].refi, num_sons);
	for(ison=0;ison<num_sons;ison++){
	  printf("%d, ",mesh->elem[iel].sons[ison]);
	}
	printf("\n");
/*kew*/
...
//  if(mmv_ref_loc.elem!=NULL){
//    free(mmv_ref_loc.elem);
//  }
//  if(mmv_ref_loc.face!=NULL){
//    free(mmv_ref_loc.face);
//  }
//  if(mmv_ref_loc.edge!=NULL){
//    free(mmv_ref_loc.edge);
//  }
//  if(mmv_ref_ali.elem!=NULL){
//    free(mmv_ref_ali.elem);
//  }
//  if(mmv_ref_ali.face!=NULL){
//    free(mmv_ref_ali.face);
//  }
//  if(mmv_ref_ali.edge!=NULL){
//    free(mmv_ref_ali.edge);
//  }
//  if(mmv_del_loc.face!=NULL){
//    free(mmv_del_loc.face);
//  }
//  if(mmv_del_loc.edge!=NULL){
//    free(mmv_del_loc.edge);
//  }
...
/*------------------------------------------------------------
  compatibility additions for hybrid module
------------------------------------------------------------*/

//------------------------------------------------------------
//  mmr_init_mesh2 - to initialize the mesh data structure for new MPI
//------------------------------------------------------------
