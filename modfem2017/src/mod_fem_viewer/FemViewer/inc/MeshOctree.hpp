...
namespace FemViewer
{
    ...
    class Octree
    {
    	typedef typename::CVec3<CoordType> WorldPoint;
        public:
            ...
            const WorldPoint& GetMin() const { return m_p0; }
            const WorldPoint& GetMax() const { return m_p1; }
        ...
        private:
            ...
            {
                p0 = WorldPoint(std::numeric_limits<CoordType>::max(), std::numeric_limits<CoordType>::max(), std::numeric_limits<CoordType>::max());
                p1 = WorldPoint(-std::numeric_limits<CoordType>::max(), -std::numeric_limits<CoordType>::max(), -std::numeric_limits<CoordType>::max());
