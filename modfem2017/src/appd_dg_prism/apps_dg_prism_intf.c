/************************************************************************
File apps_dg_prism_intf.c - implementation of the interface apph_intf.h,
                            an overlay for parallel MPI execution of
                            approximation module routines - in this case
                            for DG approximation for prismatic elements
                            (apd_dg_prism)
...
#ifdef DEBUG_APM
  if(Ent_type!=APC_ELEMENT){
    printf("DOF entity type != APC_ELEMENT in apps_dg_prism_intf! Exiting!\n");
    exit(-1);
  }
#endif
  ...
  )
{
  ...
  int mesh_id;
  mesh_id=apr_get_mesh_id(Field_id);
  mmpr_el_id_at_owner(mesh_id,Ent_id);
  return(0);
}
...
/*++++++++++++++++ executable statements ++++++++++++++++*/
  ...
  appv_exchange_table[table_id].level = 
      (appt_levels *)malloc(Nr_levels*sizeof(appt_levels));
  ...
  )
{
  ...
  int isub, iproc, nel, ipid, struct_id, owner_id, el_id_local, el_id_owner;
  double daux;
...
/*kew*/
  ...
  /* for all dof structures (for DG just elements) */
  for(istr=0;istr<Nr_dof_ent;istr++){

    nel=L_dof_ent_id[istr];
 
    owner_id = mmpr_el_owner(mesh_id, nel);
    if(owner_id != my_proc_id){

      /* set info on the dof structure */
      el_id_local = nel;
      el_id_owner = mmpr_el_id_at_owner(mesh_id, nel);
...
/*kbw
      printf("struct %d, element %d, ipid %d, owner %d, id_owner %d, nrdof %d, posg %d\n",
	     istr, nel, ipid, owner_id, el_id_owner, 
	     L_struct_nrdof[istr], L_struct_posg[istr]);
/*kew*/

      struct_id = level_p->numrecv[owner_id-1];
      level_p->dofentrecv[owner_id-1][struct_id]=el_id_owner;
...
/*kbw
  printf("Processor (subdomain) %d, level %d, elem %d (%d), nr_dofs %d s\n",
	 my_proc_id, Level_id, istr, nel, L_struct_nrdof[istr]);
...
/*kbw
	...
	printf("element ID %d (father %d), posglob %d, nrdof %d\n",
	       level_p->dofentrecv[isub][istr],
	       mmr_el_fam(mesh_id, level_p->dofentrecv[isub][istr], NULL, NULL),
...
/*kew*/
  ...
  for(iproc=0;iproc<nr_proc;iproc++){
    ...
    // should work if associated with proper message id
    //buffer_id = pcr_buffer_receive(message_id, PCC_ANY_PROC, 0);
    buffer_id = pcr_buffer_receive(message_id, iproc+1, 0);
	...
	struct_id = L_elem_to_struct[level_p->dofentsend[isub][istr]];
...
/*kbw
	...
	printf("element ID %d (father %d), posglob %d, nrdof %d\n",
	       ...
	       mmr_el_fam(mesh_id, level_p->dofentsend[isub][istr], NULL, NULL),
...
/*kbw
	...
	printf("element ID %d, posglob %d, nrdof %d\n",
	       level_p->dofentrecv[isub][ibl],
	       level_p->posrecv[isub][ibl],level_p->nrdofrecv[isub][ibl]);
	int ii;
	for(ii=level_p->posrecv[isub][ibl];
	    ii<level_p->posrecv[isub][ibl]+level_p->nrdofrecv[isub][ibl];
	    ii++){
	  printf("%20.15lf",Vec_dofs[ii]);
	}
	printf("\n");
      }
/*kew*/
  ...
  )
{
  ...
  mf_check(level_id >= 0,"appv_exchange_table[%d].nr_levels equals 0!",Table_index);
  appt_levels* level_p = &appv_exchange_table[Table_index].level[level_id];

  return(level_p->nrdof_int);
