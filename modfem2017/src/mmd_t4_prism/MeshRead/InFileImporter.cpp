...
namespace MeshRead
{
  const std::string InFileImporter::nodesBegin="NBLOCK";
  const std::string InFileImporter::elemsBegin="EBLOCK";
  const std::string InFileImporter::cmBlockBegin="CMBLOCK";
  const std::string InFileImporter::nodesEnd="N";
  
  InFileImporter::InFileImporter()
	:MeshFileImporter()
  {
	
  }

  InFileImporter::InFileImporter(const std::string & fn)
	: MeshFileImporter(fn)
  {
  }
  ...
  bool InFileImporter::Init(const std::string & filename)
  {
	_file_name = filename;
	return InFileImporter::Init();
  }
  
  bool InFileImporter::Init()
  {
	if(MeshFileImporter::Init()) {	  
	  ...
	  do{
		...
		--_elem_count;
		mmv_out_stream << " ...found " << _elem_count;
	...
	mmv_out_stream << "\nInFileImporter: initialization end." << std::endl;
	setPos(gridPos_);
	_file.clear();
	return _file.is_open();
  }

  void InFileImporter::Free()
  {
	MeshFileImporter::Free();
  }

  bool InFileImporter::doRead(hHybridMesh * mesh)
  {
	if(Init(_file_name)) {
	  assert(_file.good());
	  hHybridMesh & m(*mesh);
		  ...
		  if(block.good) {
			...
			case 8: // 8 - this is number of nodes when an element is in .in file.
			  ...
			  case ElemT4::nVerts:
                ...
                obj = m.elements_.newObj<ElemT4>(mesh,block.nodes);
				break;
			  case ElemPrism::nVerts:
                obj = m.elements_.newObj<ElemPrism>(mesh,block.nodes);
				break;
			  default:
			...
			if(obj != NULL) {
			  for(uTind i(0);i < obj->typeSpecyfic_.nFaces_; ++i) {
				m.faces_.getById(obj->components(i)).flags(B_COND)= block.field[0];
				}
			}
			++_elem_readed;
			mmv_out_stream << "\r" << _elem_readed;
		  }
		  _file >> nextChar;
		}  // we and at "-1" at end of elements block
		mmv_out_stream << "/" << _elem_count;
