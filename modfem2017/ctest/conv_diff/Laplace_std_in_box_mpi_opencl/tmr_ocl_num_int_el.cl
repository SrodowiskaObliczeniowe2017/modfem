...
kernel void tmr_ocl_num_int_el(
  ...
  __global SCALAR* stiff_mat_out, // out: result of integration of NR_ELEMS_THIS_KERCALL elements //auxiliary (obsolete) !!!!
  
  __global int* assembly_position_table,  //in: assembly table
  __global int* assembly_table,  //in: assembly table
  
  __global SCALAR* crs_val,  //out: 
  __global int *crs_col_ind, //in:
  __global int *crs_row_ptr //in:
  ...
  //printf("group_id/nr_work_groups: %d/%d ; work_group_size: %d ; thread_id: %d\n",get_group_id(0),get_num_groups(0),get_local_size(0),get_local_id(0));
  ...
  const int reader_thread = 1;
  if(get_local_id(0) >= WORK_GROUP_SIZE/2) reader_thread = 0;
  const int my_elem_num = get_local_id(0);
#endif
...
#ifdef COMPUTE_ALL_SHAPE_FUN_DER
      if(reader_thread) // part executed by reader threads (for one_el_one_thread all are readers!)
...
//********************* first loop over shape functions ***********************//
  ...
  // for 1 element per 2 threads
	...
	if(reader_thread){ // reader threads (0, 1, 2, WG_SIZE/2...) compute upper half of SM
	  idof = idof2; // idof remains the number of shape function
	} else{
	  idof = idof2 + num_shap/2; // not-reader threads compute lower half of SM
	}
#endif
...
//#ifdef ASSEMBLING
	  ...
	  for(idof=0; idof < num_shap; idof++)
	      ...
	      for(jdof=0; jdof < num_shap; jdof++)
		{
		  ...
		  write_index = assembly_table[assembly_position_table[offset+1]+idof*num_dofs+jdof];

		  #ifdef DEBUG
		  if(get_global_id(0) == 0) {
		    printf("idof=%d ; jdof=%d ; offset=%d ; write_index=%d\n",idof,jdof,offset,write_index);
		  }
		  #endif
		  
                  #ifdef USE_WORKSPACE_FOR_STIFF_MAT

		    crs_val[write_index] += workspace[my_elem_num*(num_dofs*(num_dofs+1)+PADDING)+idof*num_dofs+jdof];
		    ...
		    crs_val[write_index] += stiff_mat[idof*num_dofs+jdof];
...
//#else // if assembling disabled
...
#ifdef ONE_EL_TWO_THREADS
    // for 1 element per 2 threads
    if(!reader_thread) offset += num_shap*num_shap/2;
#endif
  ...
  // for 1 element per 2 threads
    for(idof=0; idof < num_shap/2; idof++) {
    	...
    	for(jdof=0; jdof < num_shap; jdof++)
	  {
	    ...
	    stiff_mat_out[offset+i*NR_ELEMS_PER_WORKGROUP+my_elem_num] =
	      workspace[my_elem_num*(num_dofs*(num_dofs+1)+PADDING)+idof*num_dofs+jdof];

  #else

	    stiff_mat_out[offset+i*NR_ELEMS_PER_WORKGROUP+my_elem_num] = stiff_mat[idof*num_dofs+jdof];
...
#ifdef  ONE_EL_TWO_THREADS
  // for 1 element per 2 threads
    if(!reader_thread) offset += num_shap/2;
#endif
  ...
  // for 1 element per 2 threads
    for(i=0; i < num_shap/2; i++){
      ...
      stiff_mat_out[offset+(num_shap*num_shap+i)*NR_ELEMS_PER_WORKGROUP+my_elem_num] =
	workspace[my_elem_num*(num_dofs*(num_dofs+1)+PADDING)+num_dofs*num_dofs+i];

    #else

      stiff_mat_out[offset+(num_shap*num_shap+i)*NR_ELEMS_PER_WORKGROUP+my_elem_num] = load_vec[i];
...
barrier(CLK_GLOBAL_MEM_FENCE);
 ...
 if(get_global_id(0) == 0) {
   ...
   for(ielem=0; ielem<10; ielem++) {
     printf("%d:%lf ; ",ielem,crs_val[ielem]);
